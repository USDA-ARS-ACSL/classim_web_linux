...
const fakeApi = {
  ...
  getOperations: async (tmt: number) => {
    try {
      ...
      return response.data.map((operation: { name: string; odate: string }) => ({
...
const cropManager = () => {
  ...
  const { data: crops } = useQuery({ queryKey: ["crops"], queryFn: fakeApi.getCrops })
  const { data: experiments } = useQuery({
    queryKey: ["experiments", selectedCrop],
    queryFn: () => fakeApi.getExperiments(selectedCrop!),
    enabled: !!selectedCrop,
  })
  const { data: treatments } = useQuery({
    queryKey: ["treatments", selectedExp],
    queryFn: () => fakeApi.getTreatments(selectedExp!),
    enabled: !!selectedExp,
  })
  const { data: operations } = useQuery({
    queryKey: ["operations", selectedTmt],
    queryFn: () => fakeApi.getOperations(selectedTmt!),
    enabled: !!selectedTmt,
  })
  console.log(simulationStartData)
// console.log(editableOps)
  // const mutation = useMutation({
  //   mutationFn: (data: any[]) => fakeApi.updateOperations(data),
  //   onSuccess: () => {
  //     toast({
  //       title: "Success!",
  ...
  const createExperimentMutation = useMutation({
    ...
    onError: (error: any) => {
      if (error?.body?.detail) {
        toast({
          title: "Error",
          description: error.body.detail || "Failed to create experimentw.",
          status: "error",
        });
      } else {
        toast({
      ...
      >
        ...
        <VStack align="start" spacing={4} mt={6}>
          ...
          {selectedCrop && !selectedExp && (
            <VStack align="start" spacing={4} width={"50%"}>
              <Text fontSize="lg" mb={2}>
              ...
              <HStack spacing={2} mb={4} width={"100%"}>
                ...
                />
                <Button colorScheme="blue" onClick={handleCreateExperiment} width={"50%"}>
                  Create Experiment
                </Button>
              </HStack>
          ...
          {selectedExp && !selectedTmt && (
            <VStack align="start" spacing={4} width={"50%"}>
              ...
              <HStack spacing={2} mb={4} width={"100%"}>
                ...
                />
                <Button colorScheme="blue" onClick={handleCreateTreatment} width={"50%"}>
                  Create Treatment
                </Button>
              </HStack>
          ...
          {selectedTmt && operations && (
            <VStack align="start" spacing={4}  width={"60%"}>
              <Text fontSize="lg" mb={2}>
